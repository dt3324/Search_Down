...
public class GeccoFictionListPipeline implements Pipeline<GeccoFictionList> {

    @Override
    public void process(GeccoFictionList geccoFictionList) {
        ArrayList<Document> documents = new ArrayList<>();

        //历史 军事 穿越类
        List<SimpleData> lSList = geccoFictionList.getLSList();
        for (SimpleData simpleData : lSList) {
            documents.add(simpleData.getDocument());
        }

        //灵异 科幻类
        List<SimpleData> khList = geccoFictionList.getKHList();
        for (SimpleData simpleData : khList) {
            documents.add(simpleData.getDocument());
        }

        //武侠 仙侠 修真类
        List<SimpleData> wxList = geccoFictionList.getWXList();
        for (SimpleData simpleData : wxList) {
            documents.add(simpleData.getDocument());
        }

        //言情 都市类
        List<SimpleData> yqList = geccoFictionList.getYQList();
        for (SimpleData simpleData : yqList) {
            documents.add(simpleData.getDocument());
        }

        //游戏 竞技 网游类
        List<SimpleData> yxList = geccoFictionList.getYXList();
        for (SimpleData simpleData : yxList) {
            documents.add(simpleData.getDocument());
        }

        //奇幻 玄幻类
        List<SimpleData> xhList = geccoFictionList.getXHList();
        ...
        //爬取下一页
//        HttpRequest currRequest = geccoFictionList.getRequest();
//        int currPageNum = geccoFictionList.getCurrPageNum();
